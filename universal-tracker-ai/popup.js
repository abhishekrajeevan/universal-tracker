...
// --- AI Prefill Helpers (prefill-only) ---
...
function suggestReminderForPriority(priority){
  const p = (priority||'').toLowerCase();
  const d = new Date();
  if (p==='high') return nextWeekdayAt(9);
  if (p==='medium') { d.setDate(d.getDate()+3); d.setHours(9,0,0,0); return d; }
  if (p==='low') { d.setDate(d.getDate()+7); d.setHours(9,0,0,0); return d; }
  return null;
}

// Run AI prefill after base init completes
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const resp = await new Promise((resolve,reject)=>{
      chrome.runtime.sendMessage({ type:'AI_SUGGEST', meta }, (r)=>{
